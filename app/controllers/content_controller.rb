class ContentController < AnithreController
  ...
  def show
    ...
    sw = @content.title
    amazon_dvd = Amazon::Ecs.item_search(sw, search_index: "DVD", response_group: "Medium", country: "jp")
    amazon_hobbies = Amazon::Ecs.item_search(sw, search_index: "Hobbies", response_group: "Medium", country: "jp")
  ...
  def create_params
    params.permit(:title, :subtitle, :description, :episode, :publish)
  end

  def edit_params
    params.permit(:id, :title, :subtitle, :description, :episode, :publish)
  end
